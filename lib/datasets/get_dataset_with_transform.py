...
def get_datasets(name, root, cutout):
    ...
    if name == 'cifar10' or name == 'cifar100':
        ...
        # TODO original imagenet transforms #####
        ...
        # xshape = (1, 3, 224, 224)
        # TODO original imagenet transforms #####
        # TODO simpler imagenet transforms #####
        lists = [
        ...
        if cutout > 0 : lists += [CUTOUT(cutout)]
        train_transform = transforms.Compose(lists)
        # test_transform  = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean, std)]) # TODO
        test_transform  = transforms.Compose([
            transforms.Resize((32, 32), interpolation=2),
            transforms.ToTensor(), transforms.Normalize(mean, std)
        ]) # TODO make test images the same shape as training
        xshape = (1, 3, 32, 32)
        # TODO simpler imagenet transforms #####
    else:
        raise TypeError("Unknow dataset : {:}".format(name))
    ...
    if name == 'cifar10':
        ...
        train_data = dset.CIFAR100(root, train=True , transform=train_transform, download=True)  # TODO use augmentation in train set?
        # train_data = dset.CIFAR100(root, train=True , transform=test_transform, download=True)  # TODO not to use augmentation in train set?
...
def get_nas_search_loaders(train_data, valid_data, dataset, config_root, batch_size, workers):
    ...
    if dataset == 'cifar10':
        ...
        # train_loader  = torch.utils.data.DataLoader(train_data , batch_size=batch, shuffle=True, sampler=torch.utils.data.sampler.SubsetRandomSampler(train_split), num_workers=workers, pin_memory=True)
        ...
        # search_data   = SearchDataset(dataset, search_train_data, list(range(len(search_train_data)))[:len(search_train_data)//2], list(range(len(search_train_data)))[len(search_train_data)//2:])
        search_loader = torch.utils.data.DataLoader(search_data, batch_size=batch, shuffle=True , num_workers=workers, pin_memory=True)
        train_loader  = torch.utils.data.DataLoader(train_data , batch_size=batch, shuffle=True , num_workers=workers, pin_memory=True)  # TODO shuffle in train loader?
        # train_loader  = torch.utils.data.DataLoader(train_data , batch_size=batch, shuffle=False , num_workers=workers, pin_memory=True)  # TODO not to shuffle in train loader?
