...
def get_cell_based_tiny_net(config):
    ...
    group_names = ['DARTS-V1', 'DARTS-V2', 'GDAS', 'SETN', 'ENAS', 'RANDOM', 'GDAS-meta_bn', 'ProbNAS']
    if super_type == 'basic' and config.name in group_names:
        ...
        return nas_super_nets[config.name](config.C, config.N, config.steps, config.multiplier, \
                                        config.stem_multiplier, config.num_classes, config.space, config.affine, config.track_running_stats, config.depth, config.use_stem, config.imagenet)
