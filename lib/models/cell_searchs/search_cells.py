    ...
    def forward(self, inputs, weightss):
        ...
        for i in range(1, self.max_nodes):
            ...
            for j in range(i):
                ...
                # inter_nodes.append( sum( layer(nodes[j]) * w for layer, w in zip(self.edges[node_str], weights) if w > 0.001 ) )
                inter_nodes.append( sum(layer(nodes[j]) * w if w > 0.01 else 0 for layer, w in zip(self.edges[node_str], weights)))
    ...
    def forward_prob_nas(self, inputs, index):
        ...
        for i in range(1, self.max_nodes):
            ...
            for j in range(i):
                ...
                # weights  = hardwts[ self.edge2index[node_str] ]
                argmaxs  = index[ self.edge2index[node_str] ].item()
                out = self.edges[node_str][argmaxs](nodes[j])
                # weigsum  = sum( edge(nodes[j]) if _ie == argmaxs else 0 for _ie, edge in enumerate(self.edges[node_str]) )
                inter_nodes.append( out )
            nodes.append( sum(inter_nodes) )
        return nodes[-1]
...
class MixedOp(nn.Module):
    ...
    def forward_darts(self, x, weights):
        ...
        return sum(w * op(x) if w > 0.01 else 0 for w, op in zip(weights, self._ops))

    def forward_prob_nas(self, x, index):
...
class NASNetSearchCell(nn.Module):
    ...
    def forward_prob_nas(self, s0, s1, indexs):
        s0 = self.preprocess0(s0)
        s1 = self.preprocess1(s1)
        ...
        for i in range(self._steps):
            ...
            for j, h in enumerate(states):
                ...
                # weights = weightss[ self.edge2index[node_str] ]
                index   = indexs[ self.edge2index[node_str] ].item()
                clist.append( op.forward_prob_nas(h, index) )
            states.append( sum(clist) )
