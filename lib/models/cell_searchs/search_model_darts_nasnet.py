...
class NASNetworkDARTS(nn.Module):
    ...
    def __init__(self, C: int, N: int, steps: int, multiplier: int, stem_multiplier: int,
                 ...
                 depth=-1, use_stem=True, imagenet=False):
        ...
        if not imagenet:
            ...
            self.stem = nn.Sequential(
                                            nn.Conv2d(3 if use_stem else min(3, C), C*stem_multiplier, kernel_size=3, stride=2, padding=1, bias=False),
                                            nn.BatchNorm2d(C*stem_multiplier),
                                            nn.ReLU(inplace=True),
                                            nn.Conv2d(C*stem_multiplier, C*stem_multiplier, kernel_size=3, stride=2, padding=1, bias=False),
                                            nn.BatchNorm2d(C*stem_multiplier),
                                            nn.ReLU(inplace=True),
                                            nn.Conv2d(C*stem_multiplier, C*stem_multiplier, kernel_size=3, stride=2, padding=1, bias=False),
                                            nn.BatchNorm2d(C*stem_multiplier),
    ...
    def entropy(self, mean=True):
        if mean:
            return sum(cal_entropy(arch_param, -1).mean().view(-1) for arch_param in self.get_alphas()) / len(self.get_alphas())
        else:
            return [cal_entropy(arch_param, -1) for arch_param in self.get_alphas()]
    ...
    def forward_prob_nas(self, inputs, index):
        ...
        if self.use_stem:
            ...
            s0 = s1 = inputs
        for i, cell in enumerate(self.cells):
            if cell.reduction: idx = reduce_idx
            else             : idx = normal_idx
            s0, s1 = s1, cell.forward_prob_nas(s0, s1, idx)
        out = self.lastact(s1)
        out = self.global_pooling( out )
        out = out.view(out.size(0), -1)
        logits = self.classifier(out)
