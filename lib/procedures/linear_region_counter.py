...
from datasets import CUTOUT, Lighting, Dataset2Class, ImageNet16
...
class RandChannel(object):
    ...
    def __call__(self, img):
        ...
        return torch.index_select(img, 0, torch.Tensor(channel_choice).long().to(img.device))
...
def get_datasets(name, root, input_size, cutout=-1):
    ...
    if name == 'cifar10' or name == 'cifar100':
        # lists = [transforms.RandomHorizontalFlip(), transforms.RandomCrop(input_size[1], padding=0), transforms.ToTensor(), transforms.Normalize(mean, std), RandChannel(input_size[0])]
...
class Linear_Region_Collector:
    def __init__(self, models=[], input_size=(64, 3, 32, 32), sample_batch=100, dataset='cifar100', data_path=None, sampler='uniform', std=1, seed=0):
        self.models = []
        self.std = std
    ...
    def reinit(self, models=None, input_size=None, sample_batch=None, seed=None):
        ...
        if input_size is not None or sample_batch is not None:
            ...
            if self.data_path is not None:
                ...
                self.train_loader = torch.utils.data.DataLoader(self.train_data, batch_size=self.input_size[0], num_workers=0, pin_memory=True, drop_last=True, shuffle=True)
                regions_input_data = []
                for i, (inputs, targets) in enumerate(self.train_loader):
                    if i >= self.sample_batch: break
                    regions_input_data.append((inputs, targets))
                self.train_loader = regions_input_data
